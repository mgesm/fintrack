-- Migración para incorporar investment_operations e is_investment en replace_fintrack_data
grant select, insert, update, delete on public.transaction_voids to authenticated;
...
begin
  ...
  if payload ? 'investmentOperations' then
    insert into public.investment_operations(id,symbol,product_name,product_type,side,units,unit_price,amount,operation_date,cash_account_id,linked_transaction_id,user_id)
    select x.id,x.symbol,x.product_name,x.product_type,x.side,x.units,x.unit_price,x.amount,x.operation_date,x.cash_account_id,x.linked_transaction_id,uid
    from jsonb_to_recordset(coalesce(payload->'investmentOperations','[]'::jsonb)) as x(id text,symbol text,product_name text,product_type text,side text,units numeric,unit_price numeric,amount numeric,operation_date date,cash_account_id text,linked_transaction_id text);
  end if;
end;
$$;
