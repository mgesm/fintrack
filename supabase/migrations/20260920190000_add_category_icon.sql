-- Migración para añadir columna icon a categories y actualizar replace_fintrack_data
...
begin
  ...
  insert into public.categories(id, name, color, subcats, position, archived, kind, icon, user_id)
  select x.id, x.name, x.color, coalesce(x.subcats, '[]'::jsonb), coalesce(x.position, 0), coalesce(x.archived, false), case when x.kind = 'income' then 'income' else 'expense' end, x.icon, uid
  from jsonb_to_recordset(coalesce(payload->'categories', '[]'::jsonb)) as x(id text, name text, color text, subcats jsonb, position integer, archived boolean, kind text, icon text);
