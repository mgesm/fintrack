...
| 2026-09-10 | Arquitectura simétrica y equidistante en separación entre jornadas del Libro Detallado de Movimientos (PDF): 1) Diagnóstico de corte visual resuelto por subagente Director de Arte y Maquetación: la línea previa sin separación colisionaba a apenas 1.1mm del chip de categoría (desfasado hacia arriba en la celda) y se solapaba con el fondo sombreado alterno, produciendo el efecto óptico de "cortar las entradas"; 2) Gap de respiración simétrico de 2.2mm (`dayGap = 2.2mm`) al cambiar de jornada, con línea divisoria flotando en el centro exacto (`sepLineY = (y - 4.2) + (dayGap / 2)`), garantizando una equidistancia perfecta de 1.1mm a los bordes de celda y 2.6mm a los chips superior e inferior; 3) Centrado simétrico del chip de categoría: ajuste a `pillY = y - 2.7` con 1.5mm de margen superior e inferior dentro de la celda de 7.2mm; 4) Calibración de trazo a 0.18mm en Slate-300 (`#CBD5E1`) y micro-guías intra-día a 0.10mm (`#F1F5F9`); 5) Patrón cebra por jornada (`dayTxIdx % 2 === 1`): la primera transacción de cada día se inicia siempre en blanco puro, eliminando cualquier choque de color con el separador. | `index.html`, `serviceworker.js`, `PROJECT_CONTEXT.md`. | Validación sintáctica (`node scripts/validate-syntax.js`) con 0 errores en los 4 bloques; simulación A4 y multi-página superada (`test-ledger-complete-simulation.js` y `test-new-annual-pdf.js`); versión `2026.09.10.8`, caché `v164`. |
| 2026-09-10 | Corrección de superposición visual en transición de pestañas móviles (3D Flip Glitch) y persistencia definitiva de exclusión de categorías de gasto: 1) Blindaje de visibilidad CSS en tarjetas 3D (.flip-card): resolución del aplanamiento 2D durante View Transitions y animaciones (`opacity < 1` / `transform` en `animateTabEntrance`) que causaba la superposición del reverso (calendario de gasto diario en el Donut y gráfico de barras mensual en categorías) sobre la cara frontal; se incorporaron reglas explícitas `.flip-inner:not(.flipped) .flip-back { visibility: hidden !important; pointer-events: none !important; }`, `.flip-inner:not(.flipped) .flip-front { visibility: visible; }` (y recíproco para `.flipped`), junto con `transition: visibility 0s linear .26s` para sincronizar el intercambio justo al rebasar los 90° durante giros interactivos, más un desgiro síncrono preventivo en `switchTab(i)`; 2) Persistencia permanente de exclusión de categorías de gasto: se corrigió la pérdida de selección tras reiniciar la app implementando la función centralizada `saveCategoryExpenseExclusions` que almacena síncronamente en claves global (`ft_cat_exclusions`) y de usuario (`ft_cat_exclusions_<uid>`), dispara `saveLocalCache()` a la caché cifrada en reposo y sincroniza en segundo plano con `sb.auth.updateUser` (`user_metadata`); en `loadData()` se realiza una combinación (`Object.assign`) de las exclusiones locales con las de la caché en lugar de sobreescribir, y se asegura la invalidación de memoria en `onAuthStateChange`. | `index.html`, `serviceworker.js`, `PROJECT_CONTEXT.md`. | Validación sintáctica (`node scripts/validate-syntax.js`) con 0 errores en los 4 bloques; simulación de persistencia y cache superadas; versión `2026.09.10.9`, caché `v165`. |
| 2026-09-10 | Nueva funcionalidad «No computar» en gastos e ingresos: 1) Formulario y modal de transacción: campo de importe más estrecho (`flex: 1.2`) situado en una fila compartida (`.tx-amount-row`) junto a un selector toggle idéntico al de recurrencias (`.exclude-calc-toggle`, switch ámbar y etiqueta "No computar"), visible en gastos e ingresos y auto-ocultable en transferencias (donde el importe se expande al 100%); 2) Exclusión completa del cálculo financiero: las transacciones marcadas quedan 100% excluidas de los totales de ingresos y gastos de Inicio (KPIs de balance, gráfico Donut, mapa de calor de gasto diario), presupuestos y resúmenes de Categorías, estadísticas y comparativas de la pestaña Anual, exportación Excel y resumen ejecutivo del PDF, manteniendo al mismo tiempo el descuento real del saldo bancario de la cuenta asociada (`accountTxDelta`); 3) Compatibilidad y almacenamiento Supabase sin errores de esquema: serialización con etiqueta nativa `_no_calc` en `tags` de PostgreSQL y sanitización de cargas en inserción, edición, cola offline y recurrencias (`sanitizeRemoteTxPayload`), eliminando el riesgo de errores 42703 (columna inexistente); 4) Indicador visual discreto: insignia `⊘ No computa` en gris suave y formato atenuado en el listado de transacciones y tabla del PDF (`⊘ -12,50 €`), filtrado en búsquedas e integración con accesibilidad por teclado. | `index.html`, `serviceworker.js`, `PROJECT_CONTEXT.md`. | Validación sintáctica (`node scripts/validate-syntax.js`) con 0 errores; suite de tests automatizada superada (100% de aserciones); versión `2026.09.10.10`, caché `v166`. |
| 2026-09-11 | Implementación integral de auditoría multifuncional (5 agentes especialistas), radios de diseño 20–30px y Funcionalidades 5 y 6: 1) Radios de interfaz ampliados a 20–30px: tokens actualizados en `:root` (`--radius-lg: 24px;`, `--radius-md: 20px;`, `--radius-sm: 20px;`, `--radius-xs: 18px;`, `--radius-sub: 24px;`), radios de botones de borrado de transacciones (`.tx-btn.del`), chips de inversión (`.invest-chip`), selector de rangos de activo (`.asset-range` y botones), selector de apariencia (`.theme-switch-row` y `.theme-switch-btn`) y botones de confirmación crítica (`.critical-confirm-actions button`, `.critical-confirm-counts span`, `.critical-confirm-input input`) a 20–24px; 2) Bloqueo de zoom móvil preservado al 100%: mantenimiento estricto de `user-scalable=no, maximum-scale=1.0` en cabecera y listeners de gestos táctiles por directiva explícita del usuario; 3) Funcionalidad 5: Bandeja de sincronización offline interactiva (`openSyncTray`) con Dead-Letter Queue para monitorizar incidencias de red, reintento manual inmediato y exportación de archivo JSON de rescate (`fintrack_sync_rescue_*.json`) accesible desde Ajustes con contador badge dinámico; 4) Funcionalidad 6: Exportación de Informe Anual en PDF (`exportAnnualPDF(year)`) con diseño editorial White Edition, tarjetas KPI renderizadas en Canvas Retina Hi-DPI, tabla mensual con tasas de ahorro y barras de desglose de categorías principales del año, integrable desde la pestaña Anual y el modal de exportación (`exportScope === 'year'`); 5) Correcciones críticas de auditoría: paginación PostgREST (>1000 transacciones) en `loadData`, `LOAD_MIN_MS` reducido de 1700ms a 300ms, eliminación de layout thrashing en `animateTabEntrance` mediante `requestAnimationFrame`, invalidación de cachés en `deleteAdjustment`, feedback toast en `saveBalanceAdjustment`, diálogo modal interactivo de 3 opciones al editar recurrencias con propagación de `exclude_from_calc`, protección de resurrección de series periódicas en `processRecurring`, exclusión mutua de filtro transferencias vs categoría (`setTxType`), blindaje de modo privado en gráficos anuales y operativa de inversión, y Service Worker actualizado a `fintrack-cache-v167` con Stale-While-Revalidate para carga instantánea. | `index.html`, `serviceworker.js`, `PROJECT_CONTEXT.md`. | Validación sintáctica con 0 errores en 4 bloques de script y serviceworker; versión `2026.09.11.1`, caché `v167`. |
| 2026-09-11 | Erradicación total de emojis en toda la aplicación y Reubicación de la exportación PDF Anual en Ajustes y Modal de Exportación: 1) Cero emojis en la interfaz: eliminación y sustitución integral de todos los caracteres emoji y símbolos toscos por iconos vectoriales SVG monocromáticos (`fill="none" stroke="currentColor"` o colores semánticos con opacidades suaves acordes al sistema de diseño): enlace mágico y confirmación en login (`✉`, `📬`), categorías por defecto (`🛒`, `🚗`, `🏠`, `🎮`, `💊`, `👕`, `🍽`, `📦`) migradas a claves semánticas vectoriales limpias en `CAT_ICON_PATHS` con nuevo icono para actualización de saldo (`balance`), botones de bandeja de sincronización (`↺`, `⬇`), textos descriptivos limpios en ordenación de filtros (`Fecha (reciente)`, `Fecha (antigua)`, `Importe (mayor)`, `Importe (menor)` en lugar de flechas emoji `↓`/`↑`), filas de traspaso e insignias de movimiento (`⇄`, `⊘ No computa`, `↺ Recurrente`), botones y alertas de categorías (`⚠` excedido, `🚫`/`✓` exclusión de gastos, `✎` renombrar, `↺`/`⊙` archivar/restaurar, `🔄` presupuesto recurrente automático), cuentas bancarias (`★`/`☆` cuenta predeterminada, `⇄ Transferir`), operaciones de inversión (`↗` estado vacío, flechas esbeltas de compra/venta), y botones de Ajustes (`📴`, `↺`, `🗑`, `↓`, `↑`, `⚠`); 2) Reubicación de la Exportación PDF Anual: supresión total del botón `Exportar PDF Anual` en la cabecera de la pestaña Anual (`renderAnnualTab`), restaurando el selector de año limpio `chart-year-nav`; integración de la opción en la sección «Exportar datos» de Ajustes con botón dedicado `Exportar informe anual (PDF)` que abre el modal preseleccionando el ámbito anual, e incorporación del selector dinámico de año `#exportYearWrap` en el modal `#exportModalBg` para exportar cualquier ejercicio registrado (`exportAnnualPDF(year)`). | `index.html`, `serviceworker.js`, `PROJECT_CONTEXT.md`. | Validación sintáctica con 0 errores en los 4 bloques de script embebidos y serviceworker; auditoría regex de emojis 100% limpia en index.html; versión `2026.09.11.2`, caché `v168`. |
| 2026-09-13 | Corrección definitiva del bug de resurrección de gastos recurrentes anulados / cancelados: 1) Eliminación de resurrección en `processRecurring()`: se suprimió el bucle anómalo que leía `transactionVoids` y reinsertaba movimientos anulados en la lista de recurrencias activas, el cual provocaba que al llegar la fecha de cobro se volviesen a autogenerar transacciones eliminadas o canceladas (ej. Duolingo); 2) Blindaje en cancelación de recurrencias (`deleteRecurringFromHere`): soporte robusto para transacciones sin `recur_series_id` previo, asignación de `stopFields` (`recurring: false, recur_series_id: null`) en las ocurrencias pasadas y en los metadatos de las anulaciones (`transaction_voids`), y fallback directo en Supabase si la RPC `cancel_fintrack_recurrence_from` falla; 3) Blindaje en omisión de ocurrencias individuales (`deleteOnlyRecurringOccurrence`): inserción tolerante de exclusión (`recurrence_exclusions`) con borrado seguro y soporte sin fallo ante series no indexadas. | `index.html`, `serviceworker.js`, `PROJECT_CONTEXT.md`. | Validación sintáctica con 0 errores en 4 bloques de script y serviceworker; suite de tests de recurrencias superada con éxito; versión `2026.09.13.1`, caché `v169`. |
...
|---|---|---|
| 2026-09-11 | Uso de emojis en cualquier punto de la aplicación (iconos de categorías, botones, insignias, alertas o estados). | Rechazado expresamente por el usuario ("no quiero que uses emojis en ningun sitio"). Se utilizan exclusivamente iconos vectoriales SVG monocromáticos o textos semánticos claros. |
| 2026-09-11 | Botón de exportación PDF anual en la cabecera de la pestaña Anual. | Descartado por el usuario: la cabecera de Anual conserva solo el selector de ejercicio; la exportación anual se realiza desde Ajustes y el modal de exportación. |
| 2026-09-11 | Retirar o flexibilizar el bloqueo de zoom en móviles (`user-scalable=no, maximum-scale=1.0`). | Rechazado expresamente por el usuario ("no lo retires, lo quiero asi"). El bloqueo de zoom se mantiene intacto y absoluto en todos los visores táctiles. |
| 2026-09-11 | Implementar las funciones 1 (presupuestos por subcategoría), 2 (proyección fin de mes), 3 (metas de ahorro con hucha), 4 (análisis de recurrencias huérfanas) o 7 (modo auditoría de discrepancias) del plan de auditoría. | Descartado por orden expresa del usuario: de las ideas propuestas solo se implementan exclusivamente las funciones 5 (Bandeja offline) y 6 (Exportación anual PDF). |
| 2026-09-10 | Trazar líneas divisorias de día pegadas sin margen o con grosor > 0.25mm a través de tablas con cebra global continuo en PDF. | Descartado: corta visualmente las entradas de la tabla y encajona los chips de categoría. Se adopta gap simétrico de 2.2mm con línea centrada de 0.18mm y cebra limpio reiniciado por jornada. |
